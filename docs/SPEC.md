...
Контракт backend реализован на Go и PostgreSQL. Выбор SQLite отменён командой;
API и продуктовые правила ниже сохранены. Frontend интегрируется отдельно.
...
Один Go-процесс и одна PostgreSQL-база. AI — модуль этого процесса. Frontend собирается
...
Go `net/http`, `database/sql`, драйвер `github.com/lib/pq`; без ORM.
PostgreSQL и pgAdmin запускаются через `backend/docker-compose.yml`.
...
Переменные: `PORT`, `DATABASE_URL`, `DEMO_MODE`, `AI_MODE=live|fallback`,
`OPENAI_MODEL`, `OPENAI_API_KEY`. Ключ доступен только Go-процессу; `.env` исключается из Git,
...
backend/
  ...
  internal/rating/      # B: чистая функция рейтинга
  internal/database/   # B: DB и запуск миграций
  internal/ai/          # C: провайдер, prompt, parser, fallback
  internal/offers/      # C: отклики, решения и подтверждение этапа
  internal/database/migrations/
  cmd/seed/             # импорт синтетических данных
```

Команды запуска и ограничения описаны в `backend/README.md`.
...
Таблицы: `tasks`, `teams`, `offers`, `milestones`, `progress_awards`.
`offers` — внутреннее имя предложений; основной публичный API использует `/proposals`.
Старая `point_awards` сохранена без удаления данных, но не участвует в баллах прогресса.
...
В демо `X-Demo-Actor: business:UUID` или `team:UUID` задаёт участника.
...
| `GET /business/tasks` | `limit?, offset?` | Рабочие задачи текущего владельца | B |
...
карточек, команд и откликов. Импорт: `cd backend`, затем `go run ./cmd/seed`.
Идентификаторы фикстур отображаются в стабильные UUID; существующие записи не перезаписываются.

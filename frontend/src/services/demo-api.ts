...
type Database = ReturnType<typeof initialData> & { profiles?: LocalProfile[] }
...
export class DemoApi implements Gateway {
  ...
  registerProfile(role: LocalProfile['role'], name: string): LocalProfile {
    requireText(name, 'Название профиля', 2)
    if (name.trim().length > 80)
      throw new ApiError('Название должно быть не длиннее 80 символов', 422)
    return this.write((db) => {
      const profile = { id: id(`local-${role}`), role, name: name.trim() }
      ;(db.profiles ||= []).push(profile)
      if (role === 'team')
        db.teams.push({ ...profile, interests: [], skills: [], technologies: [], points: 0 })
      return profile
    })
  }
  getProfile(profileId: string): LocalProfile | null {
    return clone(this.db.profiles?.find((profile) => profile.id === profileId) ?? null)
  }
  savedProfile(role: LocalProfile['role']): LocalProfile | null {
    return clone(
      [...(this.db.profiles ?? [])].reverse().find((profile) => profile.role === role) ?? null,
    )
  }
  ...
  async createTask(input: { draft: string; title: string; industry: Industry }) {
    if (this.actor() !== 'demo-business-1' && this.getProfile(this.actor())?.role !== 'business')
    ...
    return this.write((db) => {
      const task: Task = {
        ...
        id: id('task'),
        ownerId: this.actor(),
        ownerName: this.getProfile(this.actor())?.name ?? 'AI Sana',
        answers: [],
  ...
  async listTeams() {
    return this.db.teams.map((team) => ({
      ...clone(team),
      name:
        team.id === 'team-1' && team.name === 'Vibe Moggers' ? 'Команда веб-разработки' : team.name,
      points:

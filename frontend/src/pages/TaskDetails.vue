...
watch(loading, async (pending) => {
  if (!pending && !initialJumpDone && route.hash === '#proposal') {
...
const isOwner = computed(() => !!store.profile && task.value?.ownerId === store.actorId)
...
const milestoneErrors = reactive<Record<string, string>>({})
...
function submit(id: string, event: Event) {
  milestoneErrors[id] = validateForm(event, `milestone-error-${id}`)
  if (milestoneErrors[id]) return
  ...
  <template v-else-if="fields && score">
    <div class="detail-heading">
      ...
      <div class="detail-meta">
        <span
          ><AppIcon name="Building2" :size="15" />
          {{
            task.ownerName || (task.ownerId === 'demo-business-1' ? 'AI Sana' : 'Заказчик')
          }}</span
        ><span
    ...
    <div class="editor-layout detail-layout">
      <div class="editor-main">
        ...
        <div v-if="!store.profile && task.publishedAt" id="proposal" class="panel guest-proposal">
          <h2>Знаете, как решить эту задачу?</h2>
          <p>Представьте команду и отправьте заказчику свой план, срок и ссылку на прототип.</p>
          <RouterLink
        ...
        <template v-if="store.isTeam && task.publishedAt">
          ...
          <form
            ...
            novalidate
            @submit.prevent="submit(proposal.id, $event)"
          >
            <p
              v-if="milestoneErrors[proposal.id]"
              :id="`milestone-error-${proposal.id}`"
              class="profile-error"
              role="alert"
            >
              {{ milestoneErrors[proposal.id] }}
            </p>

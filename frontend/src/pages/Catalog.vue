...
<template>
  <section class="catalog-hero">
    <div class="hero-copy">
      <span class="eyebrow"><span class="eyebrow-dot" /> ЗАДАЧИ БИЗНЕСА × НАВЫКИ КОМАНД</span>
      <h1>Найдите команду.<br /><span>Или свой следующий проект.</span></h1>
      <p>
        Заказчики описывают задачу, команды предлагают решение.<br class="desktop-only" />
        Выберите, с чего хотите начать.
      </p>
      <div class="hero-actions">
        <RouterLink v-if="!store.isTeam" to="/tasks/new" class="button primary"
          ><AppIcon name="Plus" :size="18" />Разместить задачу</RouterLink
        >
        <a href="#catalog" class="button" :class="store.isTeam ? 'primary' : 'secondary'"
          >Найти задачу <AppIcon name="ArrowRight" :size="18"
        /></a>
      </div>
    </div>
  ...
  <section id="catalog" class="catalog-section">
    <div class="section-heading">
      <div>
        ...
        <p>Откройте карточку, изучите условия и предложите свой план решения.</p>
      </div>
    ...
    <div class="catalog-filters">
      <label class="search-field"
        ><AppIcon name="Search" :size="18" /><input
          ...
          aria-label="Поиск задач" /><button
          v-if="search"
          type="button"
          aria-label="Очистить поиск"
          @click="clearSearch"
        >
          <AppIcon name="X" :size="17" /></button></label

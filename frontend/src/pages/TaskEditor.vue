...
function clarify() {
  return perform(async () => {
    await ensureTask()
    analysis.value = await store.api.analyze({
      stage: 'clarify',
      sources: sources(),
...
function assemble() {
  return perform(async () => {
    await ensureTask()
    const result = await store.api.analyze({
      stage: 'assemble',
      sources: sources(),
      currentFields: clone(fields.value),
    })
...
function preparedQuestions() {
  return perform(async () => {
    ...
    analysis.value = {
      mode: 'fallback',
      warnings: ['AI недоступен. Вы продолжаете с подготовленными вопросами.'],
      fieldSuggestions: [],
      questions: [
        {
          id: 'q1',
          fieldKeys: ['context', 'need', 'users', 'usageScenario'],
          text: 'Как сейчас устроен процесс, что нужно изменить и кто будет пользоваться решением?',
        },
        {
          id: 'q2',
          fieldKeys: ['dataSource', 'dataFormat', 'dataAccess'],
          text: 'Какие данные доступны, в каком формате и как команда их получит?',
        },
        {
          id: 'q3',
          fieldKeys: ['deliverable', 'successMetric', 'successTarget', 'acceptanceMethod'],
          text: 'Какой результат ожидаете и как проверите его успешность?',
        },
        {
          id: 'q4',
          fieldKeys: ['deadline', 'constraints', 'contact', 'feedbackFormat'],
          text: 'Какие есть сроки и ограничения? Как связаться с бизнесом и получать обратную связь?',
        },
      ],
    }
    step.value = 2
...
onBeforeRouteLeave(
  () =>
    !dirty.value ||
    canLeave.value ||
    window.confirm('В форме есть несохранённые изменения. Покинуть страницу?'),
)
</script>
<template>
  ...
  <div v-else-if="!canEdit || (route.params.id && !task)" class="empty-state">
    ...
    <p>{{ error || 'Выберите режим бизнеса в правом верхнем углу.' }}</p>
    ...
    <div class="editor-main">
      ...
      <div v-if="error" class="inline-alert error" role="alert">
        ...
        ><button
          ...
          class="text-link"
          :disabled="busy"
          @click="preparedQuestions"
        >
          Продолжить без AI
        </button>
      </div>
      <form v-if="step === 1" class="panel editor-panel" @submit.prevent="clarify">
      ...
      <div v-else-if="step === 2" class="panel editor-panel">
        ...
        <div v-if="analysis?.mode === 'fallback'" class="inline-alert info">
          <AppIcon name="CircleHelp" :size="18" /><span
            >{{
              analysis.warnings[0] || 'AI недоступен. Используем подготовленные вопросы.'
            }}
            Ответы сохранятся вместе с карточкой.</span
          >
        </div>
        <div
        ...
        <div class="editor-actions">
          <button
            ...
            :disabled="busy"
            @click="analysis?.mode === 'fallback' ? manualCard() : assemble()"
          >
            <AppIcon
          ...
          ><button
            v-if="analysis?.mode !== 'fallback'"
            class="button ghost"
            :disabled="busy"
            @click="manualCard"
          >
            Заполнить вручную</button
          ><span v-else class="muted small">Сведения в карточку вносятся вручную</span>
        </div>
      </div>
      <div v-else>

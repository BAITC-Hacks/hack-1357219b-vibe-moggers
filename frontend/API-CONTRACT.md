> Authentication update: real requests now use HttpOnly cookies and CSRF. `X-Demo-Actor` below is legacy; see [AUTH-AI-CONTRACT.md](AUTH-AI-CONTRACT.md). AI clarification accepts only `mode: live`.
...
## Изменение UX от 23 сентября

Гостевой каталог заменил выбор готовых аккаунтов в шапке. Локальный DemoApi создаёт
отдельные профили заказчика/команды; это расширение только frontend-демо.
В режиме API пока нет создания/входа в профиль: нужны отдельный согласованный
контракт сессии и серверная авторизация. Backend в рамках изменения не трогали.
Гостевые публичные запросы используют actor `guest`; приватные отклики не запрашиваются.
...
Список ниже сохраняет прежний контракт данных, но переключателя X-Demo-Actor в UI больше нет.
...
При недоступной модели можно вернуть `mode:"fallback"`, подготовленные вопросы,
`fieldSuggestions:[]` и предупреждение. Ключ/модель находятся в окружении Go.
